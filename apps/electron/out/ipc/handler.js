...
import { IPCChannel, TaskStatus, } from '@shared/types/rpa.js';
...
export function registerIPCHandlers() {
    ...
    // ==========================================
    // 创建任务流
    ipcMain.handle(IPCChannel.TASK_CREATE, async (_event, request) => {
        try {
    ...
    // 启动任务
    ipcMain.handle(IPCChannel.TASK_START, async (_event, request) => {
        try {
    ...
    // 暂停任务
    ipcMain.handle(IPCChannel.TASK_PAUSE, async (_event, request) => {
        try {
    ...
    // 恢复任务
    ipcMain.handle(IPCChannel.TASK_RESUME, async (_event, request) => {
        try {
            const { executionId } = request.payload;
    ...
    // 停止任务
    ipcMain.handle(IPCChannel.TASK_STOP, async (_event, request) => {
        try {
            const { executionId } = request.payload;
    ...
    // 获取任务列表
    ipcMain.handle(IPCChannel.TASK_GET_LIST, async (_event, request) => {
        try {
            const db = getDatabase();
            const entities = await db.getRepository(TaskFlowEntity).find({
                order: { createdAt: 'DESC' },
            });
            const flows = entities.map((entity) => ({
                id: entity.id,
                name: entity.name,
    ...
    // 删除任务
    ipcMain.handle(IPCChannel.TASK_DELETE, async (_event, request) => {
        try {
            const db = getDatabase();
    ...
    // ==========================================
    // 获取执行状态
    ipcMain.handle(IPCChannel.EXECUTION_GET_STATE, async (_event, request) => {
        try {
            const { executionId } = request.payload;
